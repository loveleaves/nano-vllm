...
`seed`、`bad_words_token_ids`、`logprobs`。
...
各 V1 对齐轮次(A–Q)的 research/design/testing、工程优化调研报告、模型适配文档的完整索引见
...
| Phase 6 | 增强与广度对齐:M 六项增强(logprobs 端到端 / MultiProc 健壮性 / 采样 min_p·seed·bad_words / async_scheduling / swap 抢占 / metrics) · N 动态模型注册表 + 惰性加载 · O OpenAI 兼容服务入口 · P EngineCore 进程化(Inproc/MP 客户端) · Q 目录结构合理化(attention/、sample/ 上提为顶层包) |
...
| `phase6` | **当前开发分支**:增强与广度对齐(M–Q:增强项 / 模型注册表 / 服务入口 / EngineCore 进程化 / 目录合理化) |

## 测试

```bash
pytest -m unit    # 单元测试,纯 CPU,无需 GPU(304 个)
...
│   └── metadata / sampler / outputs / ops{topk_topp,penalties,logprobs,bad_words}
