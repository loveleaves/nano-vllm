...
## 背景：什么是连续批 + 为什么显式传 AttentionMetadata

**连续批（continuous batching）**：传统"静态批"要等一批请求全部到齐、对齐到同一长度再一起算，
长短请求互相等待、GPU 空转。连续批则**每步动态组批**——decode 与 prefill 混排进同一变长批，
某序列生成完即让出、新请求即补入，GPU 持续打满。这是现代推理引擎吞吐的基石。

**显式 AttentionMetadata（去全局单例）**：注意力计算需要一批"形状元数据"——每个序列的 query/KV
长度、累计偏移（cu_seqlens）、KV 物理地址（block_table）、写入位置（slot_mapping）。V0 把这些放
全局单例、靠副作用传递，难测试、难并发、难推理。V1 改为把 `AttentionMetadata` 作为**显式参数**
经 forward 链一路透传，数据流清晰、可单测、无隐藏状态。

**作用**：A（连续批）提吞吐去阶段化；B（显式元数据）让整条前向路径无全局副作用、可组合可测。
